  ...
  _createClass(Raneto, [{
    ...
    value: function getPage(filePath) {
      try {
        ...
        var plainText = _s.stripTags(_s.unescapeHTML(html));
        return {
          slug: slug,
    ...
    value: function doSearch(query) {
      ...
      results.forEach(function (result) {
        ...
        var startPoint = page.plainText.indexOf(query);
        var padding = _this3.config.excerpt_length / 2;
        var beginning = Math.max(startPoint - padding, 0);
        var end = Math.min(startPoint, _this3.config.excerpt_length, page.plainText.length);
        var leadingElipsis = beginning === 0 ? '' : '...';
        var trailingElipsis = end === page.plainText.length ? '' : '...';
        var baseExcerpt = leadingElipsis + page.plainText.substring(beginning, end) + trailingElipsis;
